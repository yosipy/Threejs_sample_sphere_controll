...
function create_skeleton_helper(model){
    ...
    let major_bone_list = make_major_skeleton_list(model);
    const key = Object.keys(major_bone_list);
    for(let i = 0; i < key.length; ++i){
        ...
        let major_bone = major_bone_list[key[i]];
...
function create_skeleton_helper(model){
    ...
    for(let i = 0, j = 0; i < key.length; ++i){
        ...
        const start_vector = new THREE.Vector3( 0 ,0 ,0 );
        vertices.push( start_vector.x, start_vector.y, start_vector.z );
        ...
        vertices.push( end_vector.x, end_vector.y, end_vector.z );
        ...
        const bone_line = new THREE.LineSegments(geometry, material);
        bone_line.frustumCulled = false;
